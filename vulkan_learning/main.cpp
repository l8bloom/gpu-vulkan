...
struct Vertex {
  ...
  bool operator==(const Vertex &otherVertex) const {
    return pos == otherVertex.pos && texCoord == otherVertex.texCoord &&
...
private:
  ...
  vk::raii::Image colorImage = VK_NULL_HANDLE;
  vk::raii::ImageView colorImageView = VK_NULL_HANDLE;
  vk::raii::DeviceMemory colorImageMemory = VK_NULL_HANDLE;

  // default value -> only 1 sample point
  ...
  void pickPhysicalDevice() {
    ...
    for (auto const &device : devices) {
      ...
      if (isSuitable) {
        ...
        std::cout << "Max MSAA level being used: " << vk::to_string(msaaSamples)
                  << std::endl;
        return;
      } else {
  ...
  void createVertexBuffer() {
    ...
    createBuffer(size, usage, memProperties, stagingBuffer,
                 stagingBufferMemory);
    // this is MMIO, first map the VRAM memory to the process's virtual
    // address space, afterward it is a simple memcpy, mapping is a sys call,
    // while memcpy is not
  ...
  void updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.model = glm::rotate(glm::mat4(1.0f), (time / 2) * glm::radians(90.0f),
  ...
  void createTextureImage() {
    ...
    std::cout << "Texture Image width: " << texWidth
              ...
              << ", number of channels: " << texChannels << std::endl;
    if (!pixels) {
    ...
    createImage(texWidth, texHeight, vk::Format::eR8G8B8A8Srgb, 1,
  ...
  void generateMipMaps(vk::raii::Image &image, vk::Format imageFormat,
                       ...
                       uint32_t mipLevels) {
    // mipmapping is rarely done during the runtime,
    // but instead as a pre step and then the layers are just loaded to VRAM as
  ...
  void createDepthResources() {
    ...
    createImage(swapChainExtent.width, swapChainExtent.height, depthFormat, 1,
  ...
  void createColorResources() {
    createImage(swapChainExtent.width, swapChainExtent.height,
                swapChainImageFormat, 1, msaaSamples, vk::ImageTiling::eOptimal,
                vk::ImageUsageFlagBits::eTransientAttachment |
                    vk::ImageUsageFlagBits::eColorAttachment,
                vk::MemoryPropertyFlagBits::eDeviceLocal, colorImage,
                colorImageMemory);
    colorImageView = createImageView(colorImage, swapChainImageFormat, 1,
                                     vk::ImageAspectFlagBits::eColor);
  }
