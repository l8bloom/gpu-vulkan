...
public:
  void run() {
    ...
    auto elapsed = std::chrono::duration<double>(time_en - time_st).count();
    std::cout << "Main loop ran for: " << elapsed << " seconds" << std::endl;
    std::cout << "Number of frames:  " << framesCount << std::endl;
    std::cout << framesCount / elapsed << " FPS" << std::endl;
    cleanup();
  }
...
private:
  ...
  vk::raii::Semaphore semaphore = VK_NULL_HANDLE;
  ...
  void createGraphicsPipeline() {
    auto shaderModule = createShaderModule(readFile("shaders/particles.spv"));
    ...
    auto vertexBindingDescription = Particle::getBindingDescription();
    auto vertexAttributeDescription = Particle::getAttributeDescription();
  ...
  void drawFrame() {
    ...
    // }
    device.resetFences(
        *inFlightFences[currentFrame]); // resetting = unsignaling
...

    {
      ...
      graphicsQueue.submit(computeSubmitInfo);
    }

    {
      ...
      // now wait until the framebuffer is both computed and attached with
      // colors
      ...
      while (device.waitSemaphores(waitInfo, UINT64_MAX) ==
             vk::Result::eTimeout) {
        ;
      }
  ...
  void createSyncObjects() {
    ...
    semaphore = vk::raii::Semaphore(
        device, vk::SemaphoreCreateInfo{.pNext = &semaphoreType});
  ...
  void updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.deltaTime = static_cast<float>(lastFrameTime * 1.0);
  ...
  void createDescriptorSets() {
    ...
    size_t readBuffer, writeBuffer;

    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      // don't forget, the updated buffer should be read by the vertex shader
      readBuffer = i == 0 ? 1 : 0;
      writeBuffer = i == 0 ? 0 : 1;
  ...
  void createImage(uint32_t width, uint32_t height, vk::Format format,
    ...
    imageMemory = vk::raii::DeviceMemory(device, memAllocInfo);
  ...
  void transitionImageLayout(vk::raii::Image &image, vk::ImageLayout oldLayout,
                             vk::ImageLayout newLayout) {
  ...
  void copyBufferToImage(const vk::raii::Buffer &buffer, vk::raii::Image &image,
    ...
    vk::BufferImageCopy regionToCopy{
        .bufferOffset = 0,
        .bufferRowLength = 0,
        .bufferImageHeight = 0,
        .imageSubresource =
    ...
    commandBuffer.copyBufferToImage(
        buffer, image, vk::ImageLayout::eTransferDstOptimal, regionToCopy);
    endSingleTimeCommands(commandBuffer);
  }

  [[nodiscard]] vk::raii::ImageView
  createImageView(vk::raii::Image &image, vk::Format format, uint32_t mipLevels,
  ...
  void createShaderStorageBuffers() {
    ...
    for (auto &particle : particles) {
      ...
      float x = r * cosf(theta) * WINDOW_HEIGHT / WINDOW_WIDTH;
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      vk::raii::Buffer storageBuffer({});
      vk::raii::DeviceMemory storageBufferMemory({});
      ...
      shaderStorageBuffers.emplace_back(std::move(storageBuffer));
      shaderStorageBuffersMemory.emplace_back(std::move(storageBufferMemory));
    }
  }
